...
import {
  BUSINESS_CONTENT_CHANNELS,
  BUSINESS_CONTENT_FORMATS,
  BUSINESS_ENTITY_LABELS,
  getBusinessContentChannelLabel,
  getBusinessContentFormatLabel,
...
export function BusinessEntitiesCenter() {
  ...
  const [contentFormat, setContentFormat] = useState<"post" | "thread" | "article" | "campaign">("post");
  const [contentGoal, setContentGoal] = useState("");
  const [contentPrimaryChannel, setContentPrimaryChannel] = useState<"x" | "telegram" | "line" | "feishu" | "wecom" | "blog">("blog");
  const [contentAccountLabel, setContentAccountLabel] = useState("主账号");
  const [contentTargetChannels, setContentTargetChannels] = useState<Array<"x" | "telegram" | "line" | "feishu" | "wecom" | "blog">>(["blog"]);
  ...
  const latestWorkflowRunByEntity = useMemo(() => {
    const nextMap: Record<string, (typeof workflowRuns)[number]> = {};
    for (const run of workflowRuns) {
      if (!run.entityType || !run.entityId) continue;
      const key = `${run.entityType}:${run.entityId}`;
      if (!nextMap[key] || run.updatedAt > nextMap[key]!.updatedAt) {
        nextMap[key] = run;
      }
    }
    return nextMap;
  ...
  const resetCreator = () => {
    ...
    setContentGoal("");
    setContentPrimaryChannel("blog");
    setContentAccountLabel("主账号");
    setContentTargetChannels(["blog"]);
    setScheduledFor("");
  };
  ...
  const submitCreate = () => {
    ...
    if (creatorType === "content") {
      createBusinessContentTask({
        ...
        channel: contentPrimaryChannel,
        format: contentFormat,
        goal: contentGoal.trim() || "完成一轮可监督的内容生产与发布准备。",
        publishTargets: (contentTargetChannels.length > 0 ? contentTargetChannels : [contentPrimaryChannel]).map(channel => ({
          channel,
          accountLabel: contentAccountLabel.trim() || "主账号",
        })),
        status: "draft",
        priority: "normal",
        ...
        <div className="control-center__mode-list">
          ...
          {creatorType === "content" && (
            <>
              ...
              <div className="scheduled-form__field">
                <label className="scheduled-form__label">内容格式</label>
                <select
                  className="input scheduled-form__input"
                  value={contentFormat}
                  onChange={event => setContentFormat(event.target.value as typeof contentFormat)}
                >
                  {BUSINESS_CONTENT_FORMATS.map(format => (
                    <option key={format} value={format}>
                      {getBusinessContentFormatLabel(format)}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="scheduled-form__field">
                ...
                <textarea
                  ...
                  placeholder="例如：面向潜在线索说明产品价值，并引导预约演示。"
                />
              </div>

              <div className="scheduled-form__field">
                <label className="scheduled-form__label">主发布渠道</label>
                <select
                  className="input scheduled-form__input"
                  value={contentPrimaryChannel}
                  onChange={event => {
                    const nextChannel = event.target.value as typeof contentPrimaryChannel;
                    setContentPrimaryChannel(nextChannel);
                    setContentTargetChannels(current => {
                      if (current.includes(nextChannel)) return current;
                      return [nextChannel, ...current].slice(0, BUSINESS_CONTENT_CHANNELS.length);
                    });
                  }}
                >
                  {BUSINESS_CONTENT_CHANNELS.map(channel => (
                    <option key={channel} value={channel}>
                      {getBusinessContentChannelLabel(channel)}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="scheduled-form__field">
                <label className="scheduled-form__label">账号标签</label>
                <input
                  className="input scheduled-form__input"
                  value={contentAccountLabel}
                  onChange={event => setContentAccountLabel(event.target.value)}
                  placeholder="例如：官网内容库 / 主账号 / 销售资料群"
                />
              </div>
              ...
              <div className="scheduled-form__field">
                ...
                <div className="control-center__theme-list">
                  {BUSINESS_CONTENT_CHANNELS.map(channel => {
                    const selected = contentTargetChannels.includes(channel);
                    return (
                      <button
                        key={channel}
                        type="button"
                        className={`btn-ghost control-center__theme-option ${selected ? "is-active" : ""}`}
                        onClick={() => {
                          setContentTargetChannels(current => {
                            if (current.includes(channel)) {
                              const next = current.filter(item => item !== channel);
                              return next.length > 0 ? next : [contentPrimaryChannel];
                            }
              ...
              <div className="scheduled-form__field">
                <label className="scheduled-form__label">计划发布时间</label>
                <input
                  className="input scheduled-form__input"
                  type="datetime-local"
                  value={scheduledFor}
      ...
      <div className="control-center__entity-grid">
        ...
        <EntitySection
          ...
          items={scopedContentTasks.map(task => {
            ...
            const linkedWorkflowRun = latestWorkflowRunByEntity[`contentTask:${task.id}`];
            return (
              <article key={task.id} className="control-center__entity-card">
                ...
                <div className="control-center__entity-meta">
                  <span>{getBusinessContentFormatLabel(task.format)} · {getBusinessContentChannelLabel(task.channel)}</span>
                ...
                <div className="control-center__entity-note">
                  发布对象: {task.publishTargets.length > 0
                ...
                {typeof task.scheduledFor === "number" ? (
                  <div className="control-center__entity-note">
                    排期: {new Date(task.scheduledFor).toLocaleString("zh-CN", { hour12: false })}
                  </div>
                ) : null}
                {task.latestDraftSummary ? (
                  <div className="control-center__entity-note">
                    最近产出: {task.latestDraftSummary}
                  </div>
                ) : null}
                {linkedWorkflowRun ? (
                  <div className="control-center__entity-note">
                    关联流程: {linkedWorkflowRun.title} · {linkedWorkflowRun.status}
                  </div>
                ) : null}
                {task.publishedLinks.length > 0 ? (
                  <div className="control-center__entity-note">
                    发布回写: {task.publishedLinks.map(link => `${link.label}${link.url ? ` (${link.url})` : link.externalId ? ` (${link.externalId})` : ""}`).join(" / ")}
                  </div>
                ) : null}
                <div className="control-center__quick-actions">
...
function getOperationLabel(operation: BusinessOperationRecord) {
  ...
  if (operation.eventType === "workflow") {
    if (operation.status === "completed") return "流程已完成";
    if (operation.status === "failed") return "流程失败";
    if (operation.status === "sent") return "流程已派发";
    return "流程处理中";
  }

  if (operation.eventType === "publish") {
    if (operation.status === "completed") return "发布成功";
